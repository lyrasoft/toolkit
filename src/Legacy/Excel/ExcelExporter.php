 ...
 * @since  1.5.13
 *
 * @deprecated  This is legacy class.
 */
class ExcelExporter
{
    ...
    {
        $file = WINDWALKER_TEMP . '/toolkit/excel/' . md5(uniqid('toolkit', true)) . '.' . $format;
    ...
    public function printHtmlTable(string $dest = 'php://output', array $options = []): void
    {
        $this->save($dest, $options, 'Html');
        die;
    }
    ...
    public function download(?string $filename = null, array $options = [], string $format = 'xlsx'): void
    {
        ...
        if (!$filename) {
            $filename = 'Export-' . Chronos::now('Y-m-d-H-i-s') . '.' . $format;
        }
